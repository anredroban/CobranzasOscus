...
class FuncionesAjaxController {
    ...
    def calcularDisponiblesAsignar(){
        ...
        if(tipoRegistros.equalsIgnoreCase("SIN GESTION")){
            ...
            String condicionSubestadoAsignacion = " and subestadoGestion.type = 'Regestionable'"
            if(subestado != ""){
                condicionSubestadoAsignacion = " and subestadoGestion.nombre = :subestado"
                condiciones.put("subestado", subestado)
            }else{
                condicionSubestadoAsignacion = " and subestadoGestion.type = 'Regestionable'"
            }
            ...
            if(tipoRegistros.equalsIgnoreCase("RELLAMADAS")){
                ...
                sql = "from Clientes where usuario.rol != :rol and nombreBase = :nombreBase and intentos != 0 and plataforma != 'PURE CLOUD' ${condicionSubestadoAsignacion} ${condicionEasyDesde} ${condicionEasyHasta} ${condicionIntentosDesde} ${condicionIntentosHasta} ${condicionSubestado}"
             //   println sql
            }
...
//Para las parroquias de las agencias
    def getParroquiasAgencias() {
        if (params.id) {
            ...
            def array = [parroquia.nombre, parroquia.nombre]
            render array as JSON
        }
    }
    ...
    def getGrafLineasMontosAcumulado(){
        Date fechaInicio = Date.parse("yyyy-MM-dd HH:mm:ss", params.fechaInicio.toString())
        Date fechaFin = Date.parse("yyyy-MM-dd HH:mm:ss", params.fechaFin.toString())
        Subestado subestados2 = Subestado.findById(3)
        def subestados = Subestado.executeQuery("from Subestado where type = 'Exitoso'")
        def adicionales = Clientes.executeQuery("select date(fechaGestion), cast(sum(valor_promesa) as integer) from Clientes where fechaGestion between :fechaInicio and :fechaFin and subestadoGestion in (:subestados) group by date(fechaGestion) order by date(fechaGestion)", [fechaInicio: fechaInicio, fechaFin: fechaFin, subestados: subestados])
        def result = [:]
        result.put(Date.parse("yyyy-MM-dd", params.fechaInicio.toString()).format("yyyy-MM-dd"), 0)
        use(TimeCategory){
            Date fecha = Date.parse("yyyy-MM-dd", params.fechaInicio.toString())
...
//        render "Hola"
   ...
   /* def getSubStatusByStatus(){
    ...
    def getSubStatusByStatus(){
        if(params.id) {
            Estado status = Estado.findById(params.id)
            def subStatus = SubSubestado.executeQuery("from SubSubestado where estado = :status and isActive = true", [status: status])
            def array = [subStatus.id, subStatus.name]
            render array as JSON
        }
    }
    ...
    def getSubSubStatusBySubStatus(){
        SubSubestado subStatus = SubSubestado.findById(params.id)
        def subSubStatus = Subestado.executeQuery("from Subestado where subSubestado = :subStatus and isActive = true", [subStatus: subStatus])
        def array = [subSubStatus.id, subSubStatus.nombre, subSubStatus.type]
        //println(array)
        render array as JSON
    }


    def getSubSubStatusBySubStatus2(){
        Subestado subStatus =  Subestado.findById(params.id.toString().toLong())
       // Subestado subStatus = Subestado.findById(idEstadoGestion)
        def substatus = Subestado.executeQuery("from Subestado where estadoPequeno = :subStatus and isActive = true", [subStatus: subStatus])
        def array = [substatus.id, substatus.nombre, substatus.type]
       // println(array + " " + subStatus + " " +  substatus)
        render array as JSON
    }
